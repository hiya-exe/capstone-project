...
def get_risk_delta(conn):
    ...
    def fetch_summary(scan_id):
        cursor.execute("SELECT severity, COUNT(*) AS count FROM findings WHERE scan_id = %s  GROUP BY severity", (scan_id,))
...
def build_business_summary(conn, scan_id):
    ...
    cursor.execute("""
        ...
        WHERE f.scan_id = %s AND f.severity IN ('Critical', 'High')
...
def dashboard():
    ...
    SELECT
        ...
        GROUP_CONCAT(c.control_code SEPARATOR ', ') AS cis_mapping
    FROM findings f
    LEFT JOIN resources r ON f.resource_id = r.resource_id
...
def api_findings():
    ...
    if service != "all":
        query += " AND r.service = %s"
        params.append(service)
    if code != "all":
        query += " AND f.finding_code = %s"
        params.append(code)
...
def remediate_finding(finding_id):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("UPDATE findings SET compliance_status = 'REMEDIATED' WHERE finding_id = %s", (finding_id,))
    conn.commit()
    conn.close()
