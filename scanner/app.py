...
# --------------------------------------------------
# SCORING ENGINE
# Deducts points per severity; clamps to 0-100.
# --------------------------------------------------
...
# --------------------------------------------------
# RISK DELTA - compares last two scans
# --------------------------------------------------
def get_risk_delta(conn):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT scan_id FROM scans ORDER BY scan_id DESC LIMIT 2"
    )
...
def dashboard():
    ...
    # All findings with CIS mappings
    cursor.execute("""
        SELECT
            f.finding_id,
            f.finding_code,
    ...
    # Scan history for trend chart (last 10, oldest first)
    cursor.execute("""
        SELECT
...
def api_findings():
    ...
    query = """
        SELECT f.finding_id, f.finding_code, f.title, f.severity,
               ...
               f.detected_at, r.service, r.aws_resource_id AS resource,
               GROUP_CONCAT(c.control_code, ', ') AS cis_mapping
        FROM findings f
        LEFT JOIN resources            r ON f.resource_id = r.resource_id
        LEFT JOIN finding_cis_mappings m ON f.finding_id  = m.finding_id
...
# --------------------------------------------------
# API - scan history for trend chart
# --------------------------------------------------
...
# --------------------------------------------------
# API - mark finding as REMEDIATED
# The dashboard passes the DB finding_id directly.
# --------------------------------------------------
...
def remediate_finding(finding_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE findings SET compliance_status = 'REMEDIATED' WHERE finding_id = ?",
        (finding_id,)
    )
    conn.commit()
    conn.close()
...
# Usage: /export?severity=Critical&service=EC2
# --------------------------------------------------
@app.route("/export")
def export_filtered():
    severity = request.args.get("severity", "all")
    ...
    query = """
        SELECT f.finding_code, f.title, f.severity, f.compliance_status,
               f.remediation, f.description, f.detected_at,
    ...
    return send_file(
        tmp_path,
        as_attachment=True,
        download_name="findings_export.json"
    )
