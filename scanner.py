...
def get_or_create_cis_control(cursor, control_code):
    cursor.execute(
        "SELECT cis_id FROM cis_controls WHERE control_code = %s",
        (control_code,)
    )
    row = cursor.fetchone()
    ...
    cursor.execute(
        "INSERT INTO cis_controls (cis_version, control_code, control_title) VALUES (%s, %s, %s)",
...
def insert_scan(cursor, account_id):
    ...
    cursor.execute(
        """
        INSERT INTO scans
        (aws_profile, aws_account_id, region, service, scan_type, started_at, completed_at, status, business_profile)
...
def insert_resource(cursor, scan_id, finding):
    cursor.execute(
        ...
        (scan_id, aws_resource_id, resource_name, resource_type, service, region, arn)
...
def insert_finding(cursor, scan_id, resource_id, finding):
    cursor.execute(
        ...
        (scan_id, resource_id, finding_code, title, description, severity, compliance_status,
         remediation, evidence, detected_at, business_framework, business_mapping,
         db_engine, db_environment, db_engine_note)
        ...
        (
            ...
            finding["remediation"],
            to_db_text(finding.get("evidence")),
            finding["detected_at"],
            finding.get("business_framework"),
            finding.get("business_mapping"),
            finding.get("db_engine"),
            finding.get("db_environment"),
            finding.get("db_engine_note")
        )
    )
...
def insert_finding_cis_mappings(cursor, finding_id, cis_controls):
    for control_code in cis_controls.split(", "):
        ...
        cursor.execute(
            "INSERT INTO finding_cis_mappings (finding_id, cis_id) VALUES (%s, %s)",
            (finding_id, cis_id)
        )
...
def save_findings_to_db(findings):
    ...
    print(f"Findings saved to MySQL database: {DB_CONFIG['database']} (scan_id={scan_id})")
...
def print_findings(findings):
    ...
    for f in findings:
        ...
        print(f"Risk : {f['risk']}")
        print(f"CIS Mapping : {f['cis_mapping']}")
